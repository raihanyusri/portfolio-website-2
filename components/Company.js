    ...
    return (
        <div className="flex flex-wrap-reverse items-end justify-evenly pt-20">
            <div className="px-6 h-full lg:w-2/3">
                ...
                <div>
                    {props.desc.map(desc => <p className="text-lg text-gray-700 text-base">• {desc}</p>)}            
                </div>
                <div className="pt-4 pb-2">
                    {props.skills.map(skill =>
                        <span className="inline-block bg-beige rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">{skill}</span>
                        )}
                </div>
            </div>
            <div>
